...
**Goal:** Replace the hardcoded static ApexCharts demo on the company dashboard with a live dashboard showing company profile details and 5 bar charts of resource counts grouped by category.

**Architecture:** Single JSON endpoint from `DashboardsController#index` returns company details + category-grouped counts for Products, Stocks, Services, Orders, Employees. Frontend fetches JSON on connect and renders a company profile card + 5 ApexCharts bar charts.

**Tech Stack:** Rails 7+ JSON API, Stimulus, ApexCharts (already pinned in importmap)
...
|------|--------|----------------|
| `app/controllers/companies/dashboards_controller.rb` | Modify | Add `format.json` with company details + 5 category-grouped counts |
| `app/javascript/controllers/companies/dashboards/index_controller.js` | Rewrite | Fix class name, fetch real data, render company card + 5 ApexCharts |
| `app/policies/companies/dashboards_policy.rb` | No change | `index?` → `true` is sufficient |

---

### Task 1: Add JSON endpoint to DashboardsController

**Files:**
- Modify: `app/controllers/companies/dashboards_controller.rb` (full rewrite)

- [ ] **Step 1: Rewrite the controller**

Replace the current controller body with Shell-First pattern including `format.json`:
...
Run: `bin/rubocop --only Syntax app/controllers/companies/dashboards_controller.rb`
Expected: No offenses

- [ ] **Step 3: Commit**

```bash
git add app/controllers/companies/dashboards_controller.rb
git commit -m "feat: add JSON endpoint to dashboard with company details and category-grouped counts"
```

---

### Task 2: Rewrite Dashboard Stimulus Controller

**Files:**
- Modify: `app/javascript/controllers/companies/dashboards/index_controller.js` (full rewrite)
...
export default class Companies_Dashboards_IndexController extends Companies_LayoutController {
  ...
  renderCharts() {
    ...
    resources.forEach(({ key, label, icon }) => {
...
Run: `bin/rails assets:clobber && bin/rails assets:precompile 2>&1 | tail -5`
Expected: No JS syntax errors

- [ ] **Step 3: Commit**

```bash
git add app/javascript/controllers/companies/dashboards/index_controller.js
git commit -m "feat: rewrite dashboard controller with company profile and live ApexCharts"
```
...
```bash
bin/rails spec:features 2>&1 | tail -20
```
