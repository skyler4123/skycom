...
| `after_create :setup_chatwoot_account` | 167 | `setup_chatwoot_account` | Provisions one Chatwoot account per company via `Chatwoot::BaseService.create_account!(company: self)` and stores the returned id on `companies.chatwoot_account_id`. Deliberately **outside** the `skip_init`/`system_owned` guard — normal and system companies both get accounts. Rescues all errors with a `[Chatwoot]` error log so a down Chatwoot never blocks company creation. The Chatwoot account is never deleted when the company is destroyed. See `docs/CHATWOOT.md`. |
...
| `after_create` | 7 | Category, Company, Branch, PolicyAppointment, PropertyMapping, RoleAppointment |
...
*Last updated: 2026-09-17. Auto-generated by codebase exploration.*
