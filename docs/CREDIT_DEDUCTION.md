...
> **Status**: Live. Credit deduction is controlled at the **controller action level** via an `after_action` filter, and the deduction logic lives in a dedicated service class hierarchy (`Deduct::*`).
...
2. **Deduction is a filter concern; the logic belongs to a service class.** Each controller *declares* which `Deduct::*` service runs for which action. The `after_action` filter executes the service after a successful JSON response.
...
Because rules differ per place even for the same action type, each (controller, action) pair gets its **own subclass** of the epic `Deduct::BaseService`. The subclass controls cost, description, and guards; the base class owns the deduction core (priority order + metering).
...
| **Never breaks the request** | Any error inside the deduction is rescued and logged as `[CreditDeduction]` — the page always renders |
...
## 3. The Deduct Service Hierarchy (Service Layer)
...
└── companies/
    └── dashboards/
        └── index_service.rb                        # Deduct::Companies::Dashboards::IndexService
```
...
| URL | Service class |
|-----|---------------|
| `/companies/:company_id/dashboards` + `index` | `Deduct::Companies::Dashboards::IndexService` |
| `/companies/:company_id/orders` + `create` | `Deduct::Companies::Orders::CreateService` (future) |

### 3.1 `Deduct::BaseService` — The Epic

One purpose: **move credits out of the wallet for one action and meter the usage.**

```ruby
class Deduct::Companies::Dashboards::IndexService < Deduct::BaseService
...
The wallet stores credit balance **numbers only** (atomic-purpose pattern — no business logic). The `Deduct::*` services own the priority rules.
...
- `deduct_from!` raises `CompanyWallet::InsufficientCreditsError` only when that **single** balance lacks funds — the `Deduct::BaseService` orchestrates across balances so this never surfaces in the deduction path.
...
```ruby
# app/services/deduct/companies/dashboards/export_service.rb
module Deduct
  module Companies
    module Dashboards
      class ExportService < Deduct::BaseService
...
```ruby
class Companies::DashboardsController < Companies::ApplicationController
  deduct_credits_for :export, with: Deduct::Companies::Dashboards::ExportService
end
```
...
```ruby
# spec/services/deduct/companies/dashboards/export_service_spec.rb
...
| File | Purpose |
|------|---------|
| `app/controllers/concerns/companies/credit_deduction_concern.rb` | `after_action :run_credit_deduction` + `deduct_credits_for` DSL |
| `app/controllers/companies/application_controller.rb` | Includes the concern for all company controllers |
| `app/services/deduct/base_service.rb` | Epic service: priority deduction + metering, subclass contract |
| `app/services/deduct/companies/dashboards/index_service.rb` | Dashboard-access deduction (reference implementation) |
...
| `spec/services/deduct/base_service_spec.rb` | Priority/deduction/metering coverage |
| `spec/services/deduct/companies/dashboards/index_service_spec.rb` | Dashboard service contract |
...
- `docs/ATOMIC_PURPOSE.md` — the credit chain (`CompanyTransaction → CompanyInvoice → CompanyOrder → CompanyWallet`) and the atomic-purpose pattern; the wallet's per-balance entry points are called by `Deduct::*` services
