...
**Numeric standard columns (`quantity`, `pending`):** beyond `property_*` slots, the plain metric
columns in `TableConfig::NUMERIC_STANDARD_KEYS` are range-filterable too. `field_hash` seeds them
with the same integer range buckets, and `infer_column_type` treats them as `integer`. For the
filters to actually execute in Meilisearch, the indexed model must also declare them via
`ms_extra_filterable_columns` (see `docs/MEILISEARCH.md` §2.1) — `Stock`/`StockTransfer`/
`StockImport`/`StockExport` do.
