...
RSpec.describe "article_group_appointments/edit", type: :view do
  let(:article_group_appointment) {
    ArticleGroupAppointment.create!(
      article_group: nil,
      appoint_from: nil,
      appoint_to: nil,
      appoint_for: nil,
  ...
  before(:each) do
    assign(:article_group_appointment, article_group_appointment)
  end

  it "renders the edit article_group_appointment form" do
    render

    assert_select "form[action=?][method=?]", article_group_appointment_path(article_group_appointment), "post" do
