...
RSpec.describe PropertyMapping, type: :model do
  describe "callbacks" do
    describe "after_create :create_default_table_config" do
      ...
      it "creates a TableConfig with default columns_metadata" do
        ...
        expect(tc.metadata["columns"]).to be_present
        expect(tc.metadata["columns"]).to be_an(Array)
        expect(tc.metadata["columns"].first["key"]).to eq("name")
      end
    end
  end
  ...
  describe "after_update :sync_table_configs" do
    ...
    it "adds a new column when a property is added" do
      ...
      table_config.reload
      keys = table_config.metadata["columns"].map { |c| c["key"] }
      expect(keys).to include("property_boolean_1")
      new_col = table_config.metadata["columns"].find { |c| c["key"] == "property_boolean_1" }
    ...
    it "updates column name when a property is renamed" do
      ...
      table_config.reload
      col = table_config.metadata["columns"].find { |c| c["key"] == "property_string_1" }
    ...
    it "does not affect non-property columns" do
      ...
      table_config.reload
      expect(table_config.metadata["columns"].map { |c| c["key"] }).to include("name")
    end
    ...
    it "syncs all associated table configs" do
      ...
      tc2.reload
      expect(tc2.metadata["columns"].map { |c| c["key"] }).to include("property_boolean_1")
    end
