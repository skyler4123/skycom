...
RSpec.feature "Companies::StockExports dynamic search/filter", type: :feature, js: true do
  let(:company) { create(:company) }
  let(:owner) { company.user }
  let(:category) { Seed::CategoryService.find_or_create_for(company: company, resource_name: "stock_exports") }
  ...
  let!(:table_config) do
    ...
    TableConfig.create!(company: company, category: category,
      property_mapping: category.default_property_mapping, resource_name: "stock_exports",
      metadata: { "columns" => [
  ...
  let!(:small) do
    ...
    create(:stock_export, company: company, category: category, warehouse: warehouse, product: product,
  ...
  after { StockExport.ms_clear_index! }

  scenario "renders search input and filter dropdown from TableConfig" do
    visit company_stock_exports_path(company, category_id: category.id)
  ...
  scenario "keyword search via GET form narrows the table" do
    visit company_stock_exports_path(company, category_id: category.id, q: "Crimson")
  ...
  scenario "range bucket filters results on the quantity metric" do
    visit company_stock_exports_path(company, category_id: category.id, "filters" => { "quantity" => "100:" })
