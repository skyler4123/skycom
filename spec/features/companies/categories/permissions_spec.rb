...
RSpec.feature "Companies::Categories Permissions", type: :feature, js: true do
  ...
  scenario "employee with update permission can edit property select label via show modal" do
    editor_employee.clear_permissions_cache
    company.clear_permissions_cache
    editor_employee.reload
    ...
    section = find('h4', text: 'Property Select Fields (1-20)').find(:xpath, '..')
    property_editable = section.find('[data-controller="editable"]', match: :first)
    property_editable.click

    expect(page).to have_selector('.editable-input', wait: 5)

    property_editable.find('.editable-input').fill_in(with: 'Seasonal Drop')

    accept_confirm do
      property_editable.find('.editable-input').send_keys :enter
    end

    expect(page).to have_content("property_select_1 updated!", wait: 10)

    target_category.property_mapping.reload
    expect(target_category.property_mapping.property_select_1).to eq('Seasonal Drop')
  end

  scenario "employee with update permission can edit property text label via show modal" do
    editor_employee.clear_permissions_cache
    company.clear_permissions_cache
    editor_employee.reload
    ...
    property_editable.find('.editable-input').fill_in(with: 'Full Description')
    ...
    target_category.property_mapping.reload
    expect(target_category.property_mapping.property_text_1).to eq('Full Description')
  end

  scenario "employee with update permission can edit property integer label via show modal" do
    editor_employee.clear_permissions_cache
    company.clear_permissions_cache
    editor_employee.reload
    ...
    property_editable.find('.editable-input').fill_in(with: 'Quantity in Stock')
    ...
    target_category.property_mapping.reload
    expect(target_category.property_mapping.property_integer_1).to eq('Quantity in Stock')
  end

  scenario "employee with update permission can edit property decimal label via show modal" do
    editor_employee.clear_permissions_cache
    company.clear_permissions_cache
    editor_employee.reload
    ...
    property_editable.find('.editable-input').fill_in(with: 'Weight (kg)')
    ...
    target_category.property_mapping.reload
    expect(target_category.property_mapping.property_decimal_1).to eq('Weight (kg)')
  end

  scenario "employee with update permission can edit property boolean label via show modal" do
    editor_employee.clear_permissions_cache
    company.clear_permissions_cache
    editor_employee.reload
    ...
    section = find('h4', text: 'Property Boolean Fields (1-20)').find(:xpath, '..')
    property_editable = section.find('[data-controller="editable"]', match: :first)
    property_editable.click

    expect(page).to have_selector('.editable-input', wait: 5)

    property_editable.find('.editable-input').fill_in(with: 'Is Featured')
    ...
    target_category.property_mapping.reload
    expect(target_category.property_mapping.property_boolean_1).to eq('Is Featured')
  end

  scenario "employee with update permission can edit property datetime label via show modal" do
    editor_employee.clear_permissions_cache
    company.clear_permissions_cache
    editor_employee.reload
    ...
    property_editable.find('.editable-input').fill_in(with: 'Start Date')
    ...
    target_category.property_mapping.reload
    expect(target_category.property_mapping.property_datetime_1).to eq('Start Date')
  end
