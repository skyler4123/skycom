...
RSpec.feature "Companies::Facilities Permissions", type: :feature, js: true do
  ...
  scenario "employee with update permission can see table with edit links" do
    ...
    property_mapping.update!(
      metadata: { "properties" => [
        { "key" => "property_string_1", "type" => "string", "name" => "Location" }
      ] }
    )
    table_config.update!(
      metadata: { "columns" => [
        { "key" => "property_string_1", "name" => "Location", "visible" => true, "sortable" => true, "align" => "left", "pinned" => nil, "width" => nil, "roles" => [], "is_virtual" => false, "render_config" => {} }
      ] }
    )
    target_facility.update!(property_string_1: "Floor 2")

    seed_client_cache
    visit company_facilities_path(company, category_id: category.id)
  ...
  scenario "editor with update permission can update facility name via edit page" do
    ...
    target_facility.category.default_property_mapping.update!(metadata: { "properties" => [] })
    sign_in(editor_user)
    page.execute_script("localStorage.clear()")
