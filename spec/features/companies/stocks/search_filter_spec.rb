...
RSpec.feature "Companies::Stocks dynamic search/filter", type: :feature, js: true do
  ...
  let!(:small) do
    ...
    create(:stock, company: company, category: category, warehouse: warehouse, product: product,
  ...
  after { Stock.ms_clear_index! }

  scenario "renders search input and filter dropdown from TableConfig" do
    visit company_stocks_path(company, category_id: category.id)
  ...
  scenario "keyword search via GET form narrows the table" do
    visit company_stocks_path(company, category_id: category.id, q: "Crimson")
  ...
  scenario "range bucket filters results on the quantity metric" do
    visit company_stocks_path(company, category_id: category.id, "filters" => { "quantity" => "100:" })
