...
def seed_client_cache
  ...
  scenario "employee without create permission cannot access new page" do
    ...
    toggle_policy(role_text: "NoPermission", action: "read", resource: "Brand")
  ...
  scenario "employee with update permission can see table with edit links" do
    ...
    property_mapping.update!(
      metadata: { "properties" => [
        { "key" => "property_string_1", "type" => "string", "name" => "Tier" }
      ] }
    )
    table_config.update!(
      metadata: { "columns" => [
        { "key" => "property_string_1", "name" => "Tier", "visible" => true, "sortable" => true, "align" => "left", "pinned" => nil, "width" => nil, "roles" => [], "is_virtual" => false, "render_config" => {} }
      ] }
    )
    target_brand.update!(property_string_1: "Luxury")

    seed_client_cache
    visit company_brands_path(company, category_id: category.id)
  ...
  scenario "editor with update permission can update brand name via edit page" do
    ...
    target_brand.category.default_property_mapping.update!(metadata: { "properties" => [] })
    sign_in(editor_user)
    page.execute_script("localStorage.clear()")
  ...
  scenario "employee without update permission cannot access edit page" do
    ...
    unless editor_section.has_css?('.resource-section', text: 'Brand')
  ...
  scenario "policy appointment workflow_status can be toggled to grant permission" do
    ...
    toggle_policy(role_text: "NoPermission", action: "create", resource: "Brand")
