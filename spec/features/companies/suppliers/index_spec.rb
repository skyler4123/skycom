...
RSpec.feature "Companies::Suppliers Management", type: :feature, js: true do
  let(:company) { create(:company) }
  let(:owner) { company.user }

  let!(:default_category) do
    Seed::CategoryService.find_or_create_for(company: company, resource_name: "suppliers")
  end

  let!(:supplier) do
    create(:supplier, company: company, category: default_category,
      name: "Alpha Supplier #{SecureRandom.hex(4)}", business_type: "manufacturer")
  end
  let!(:supplier2) do
    create(:supplier, company: company, category: default_category,
      name: "Beta Supplier #{SecureRandom.hex(4)}", business_type: "distributor", workflow_status: "pending")
  end
  ...
  let!(:default_table_config) do
    ...
    TableConfig.create!(
      ...
      metadata: { "columns" => [
        { "key" => "name", "name" => "Supplier Name", "visible" => true, "align" => "left", "width" => nil },
  ...
  scenario "index page loads and displays suppliers table" do
    visit company_suppliers_path(company)

    expect(page).to have_selector('table', wait: 10)

    expect(page).to have_selector('th', text: 'Supplier Name')
  ...
  scenario "edit button links to edit page for supplier" do
    visit company_suppliers_path(company)
    expect(page).to have_selector('table', wait: 10)
  ...
  scenario "filter by category updates URL and filters table" do
    category = Seed::CategoryService.create(company: company, name: "Test Category", resource_name: "suppliers")
    supplier.update!(category: category, property_mapping: category.default_property_mapping)
    visit company_suppliers_path(company)
    expect(page).to have_selector('table', wait: 10)
  ...
  scenario "display supplier workflow status as badge" do
    visit company_suppliers_path(company)
    expect(page).to have_selector('table', wait: 10)
  ...
  describe "table title" do
    let(:test_category) do
      Seed::CategoryService.create(company: company, name: "Test Category", resource_name: "suppliers")
    end
    ...
    scenario "shows table title with resource name and category name" do
      visit company_suppliers_path(company, category_id: test_category.id)
      expect(page).to have_selector('table', wait: 10)

      expect(page).to have_selector('h2', text: /Suppliers - Test Category/)
    end
