...
RSpec.feature "Companies::Departments Permissions", type: :feature, js: true do
  ...
  scenario "creator can create new department and see in table" do
    ...
    fill_in 'department[email]', with: 'creator@department.com'
  ...
  # =========================================================================
  scenario "employee without create permission cannot create new department" do
    sign_in(owner)
    ...
    no_permission_section = find('.role-section', text: "NoPermission")
    read_label = no_permission_section.all('label').find { |l| l.text.include?("Can read Department") }
    ...
    expect(page).to have_selector('form[data-action*="handleSubmit"]', wait: 10)
    fill_in 'department[name]', with: 'Should Not Be Created'
    fill_in 'department[email]', with: 'test@example.com'
    select 'Operations', from: 'department[business_type]'
    ...
    expect(Department.find_by(name: "Should Not Be Created")).to be_nil
  end
  ...
  # =========================================================================
  # SCENARIO 3b: Editor can update department name via show modal
  # =========================================================================
  scenario "editor with update permission can update department name via editable" do
    sign_in(owner)
    visit company_departments_path(company)

    expect(page).to have_selector('table', wait: 10)

    target_row = find('tbody tr', text: target_department.name)
  ...
  # =========================================================================
  # SCENARIO 3c: Editor can update department description via show modal
  # =========================================================================
  scenario "editor with update permission can update department description via editable" do
    sign_in(owner)
    visit company_departments_path(company)
    ...
    expect(page).to have_selector('tbody tr', wait: 10)
    expect(Department.find_by(id: target_department.id).description).to eq("Updated description")
  end

  # =========================================================================
  # SCENARIO 3d: Employee WITHOUT update permission gets error when editing
  # =========================================================================
  scenario "employee without update permission cannot edit another department's name" do
    sign_in(owner)
    visit company_permissions_path(company)

    editor_section = find('.role-section', text: "Editor")

    unless editor_section.has_content?("Can update Department")
  ...
  scenario "policy appointment workflow_status can be toggled to grant permission" do
    ...
    no_permission_section = find('.role-section', text: "NoPermission")
    create_label = no_permission_section.all('label').find { |l| l.text.include?("Can create Department") }
