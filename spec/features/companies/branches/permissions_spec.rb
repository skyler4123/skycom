...
def seed_client_cache
  ...
  scenario "employee with update permission can see table with edit links" do
    ...
    property_mapping.update!(
      metadata: { "properties" => [
        { "key" => "property_string_1", "type" => "string", "name" => "Zone" }
      ] }
    )
    table_config.update!(
      metadata: { "columns" => [
        { "key" => "property_string_1", "name" => "Zone", "visible" => true, "sortable" => true, "align" => "left", "pinned" => nil, "width" => nil, "roles" => [], "is_virtual" => false, "render_config" => {} }
      ] }
    )
    target_branch.update!(property_string_1: "North")

    seed_client_cache
    visit company_branches_path(company, category_id: category.id)
  ...
  scenario "editor with update permission can update branch name via edit page" do
    ...
    target_branch.category.default_property_mapping.update!(metadata: { "properties" => [] })
    sign_in(editor_user)
    page.execute_script("localStorage.clear()")
