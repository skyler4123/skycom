...
RSpec.feature "Companies::StockTransfers dynamic search/filter", type: :feature, js: true do
  ...
  let!(:small) do
    ...
    create(:stock_transfer, company: company, category: category, warehouse: warehouse, product: product,
  ...
  after { StockTransfer.ms_clear_index! }
  ...
  scenario "keyword search via GET form narrows the table" do
    visit company_stock_transfers_path(company, category_id: category.id, q: "Crimson")
  ...
  scenario "range bucket filters results on the quantity metric" do
    visit company_stock_transfers_path(company, category_id: category.id, "filters" => { "quantity" => "100:" })
