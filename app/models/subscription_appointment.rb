class SubscriptionAppointment < ApplicationRecord
  ...
  validate :system_cannot_be_buyer
  ...
  def system_cannot_be_buyer
    # 1. Check by Class
    if appoint_to.is_a?(System)
      errors.add(:appoint_to, "The System cannot buy subscriptions.")
    end

    # 2. Check by specific Code (Double Safety)
    if appoint_to.respond_to?(:code) && appoint_to.code == "System"
      errors.add(:appoint_to, "The System cannot buy subscriptions.")
    end
  end
end
