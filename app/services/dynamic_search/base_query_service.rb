...
# app/services/dynamic_search/base_query_service.rb

# Generic core of the TableConfig-driven dynamic search/filter (docs/DYNAMIC_TABLE.md §2.5).
# Subclasses declare the searchable model + a fallback resource_name for config resolution:
#
...
#   end
#
# Contract (identical on every subclass):
#   service = Subclass.new(company:, params:)
...
class DynamicSearch::BaseQueryService
  ...
  # Subclasses must override: the ActiveRecord model indexed via DynamicSearchConcern.
  def self.model
    raise NotImplementedError, "#{name} must define self.model"
  end

  # Subclasses may override: TableConfig resource_name used when no category_id is given.
  def self.fallback_resource_name
    nil
  end
  ...
  def table_config
    ...
    elsif self.class.fallback_resource_name.present?
      # Fallback: no category selected — use the resource's default config if one exists.
      company.table_configs.find_by(resource_name: self.class.fallback_resource_name)
    end
  end
