class ProductBrandsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @product_brand.update(product_brand_params)
        format.html { redirect_to @product_brand, notice: "Product brand was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to product_brands_path, notice: "Product brand was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_product_brand
      @product_brand = ProductBrand.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def product_brand_params
      params.expect(product_brand: [ :name, :description, :status, :business_type, :discarded_at ])
    end
end
