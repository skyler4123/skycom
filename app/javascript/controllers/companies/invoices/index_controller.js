...
export default class Companies_Invoices_IndexController extends Companies_LayoutController {
  // Invoices dashboard — table hydrates from the index JSON of the current URL.
  // Search/filter controls render from the active TableConfig via the shared helpers
  // (dynamicSearchHTML / dynamicFiltersHTML).
  // Depends on BE: Companies::InvoicesController#index (list + Meilisearch q / filters[key])
  ...
  contentHTML() {
    ...
    const fallbackColumns = [
      { key: "name", name: translate("Invoice Name") },
      { key: "code", name: translate("Invoice Code") },
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <form method="get" action="${pathname()}" class="flex flex-col lg:flex-row items-end justify-between gap-4 w-full">
              ...
              <a href="${Helpers.new_company_invoice_path(currentCompany().id)}"
            ...
            ${table({
                ...
                name: (value, record) => `
                  <div class="flex items-center gap-4">
                    <div class="w-8 h-8 rounded-lg bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center shrink-0">
                      <span class="material-symbols-outlined text-orange-600 dark:text-orange-400 text-[18px]">receipt</span>
                    </div>
                    <a href="${Helpers.company_invoice_path(currentCompany().id, record.id)}"
                      ...
                      ${value || translate("Unnamed Invoice")}
                    </a>
                  </div>
                `,
              ...
              renderActions: (record) => `
                <td class="py-4 px-6 text-sm text-right whitespace-nowrap">
                  <a href="${Helpers.edit_company_invoice_path(currentCompany().id, record.id)}"
