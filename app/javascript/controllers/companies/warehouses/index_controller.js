...
export default class Companies_Warehouses_IndexController extends Companies_LayoutController {
  // Warehouses dashboard — table hydrates from the index JSON of the current URL.
  // Search/filter controls render from the active TableConfig via the shared helpers
  // (dynamicSearchHTML / dynamicFiltersHTML).
  // Depends on BE: Companies::WarehousesController#index (list + Meilisearch q / filters[key])
  ...
  async connect() {
    ...
    try {
      ...
      toast({ type: "error", message: `${ translate("Failed to load warehouses") }${__errDetail ? ": " + __errDetail : ""}` })
    }
  ...
  contentHTML() {
    ...
    const fallbackColumns = [
      { key: "name", name: translate("Warehouse Name") },
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <form method="get" action="${pathname()}" class="flex flex-col lg:flex-row items-end justify-between gap-4 mb-6 w-full">
              ...
              <a href="${Helpers.new_company_warehouse_path(currentCompany().id)}"
            ...
            ${table({
              ...
              renderers: {
                name: (value, record) => `
                  <div class="flex items-center gap-4">
                    ...
                    <a href="${Helpers.company_warehouse_path(currentCompany().id, record.id)}"
                      ...
                      ${value || translate("Unnamed Warehouse")}
                    </a>
                  </div>
                `,
              ...
              renderActions: (record) => `
                <td class="py-4 px-6 text-sm text-right whitespace-nowrap">
                  <a href="${Helpers.edit_company_warehouse_path(currentCompany().id, record.id)}"
