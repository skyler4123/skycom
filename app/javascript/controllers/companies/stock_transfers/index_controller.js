...
export default class Companies_StockTransfers_IndexController extends Companies_LayoutController {
  ...
  async connect() {
    ...
    try {
      ...
      toast({ type: "error", message: "Failed to load stock transfers" })
    }
  }
  ...
  contentHTML() {
    const typeFilter = Enums()?.stock_transfer?.business_types || []
    const workflowStatusFilter = Enums()?.stock_transfer?.workflow_statuses || []
