...
import Companies_Departments_NewModalController from "controllers/companies/departments/new_modal_controller";
import Companies_Departments_ShowModalController from "controllers/companies/departments/show_modal_controller";
...
export default class Companies_Departments_IndexController extends Companies_LayoutController {
  ...
  openNewModal() {
    openModal({ html: `<div data-controller="${identifier(Companies_Departments_NewModalController)}"></div>` })
  }
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                  <th class="py-4 px-6 font-medium whitespace-nowrap">Department Name</th>
              ...
              <tbody data-${this.identifier}-target="departmentsList" class="divide-y divide-slate-200 dark:divide-slate-800">
                ${this.departments.map(department => `
                  <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td class="py-4 px-6 text-sm">
                      <div class="flex items-center gap-4">
                        <div class="w-10 h-10 rounded-lg bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center">
                          <span class="material-symbols-outlined text-indigo-600 dark:text-indigo-400">corporate_fare</span>
                        </div>
                        <div>
                    ...
                    <td class="py-4 px-6 text-sm text-slate-600 dark:text-slate-300">${department.category?.name || 'N/A'}</td>
                    <td class="py-4 px-6 text-sm">
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        department.business_type === 'sales' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' :
                        department.business_type === 'marketing' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300' :
                        department.business_type === 'finance' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300' :
                        'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300'
                      }">
                        ${Helpers.capitalize(department.business_type?.replace('_', ' ') || 'sales')}
                      </span>
                    </td>
                    <td class="py-4 px-6 text-sm">
                      ${Helpers.statusBadge(department.workflow_status)}
                    </td>
                    <td class="py-4 px-6 text-sm text-right">
                      <button
                        ...
                        data-${this.identifier}-department-id-param="${department.id}"
                      >
