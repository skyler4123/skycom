...
export default class Companies_Departments_NewController extends Companies_LayoutController {
  /** @type {string | null} */
  categoryId = null
    ...
    switch (type) {
      case 'boolean':
        return `
          <div class="flex items-center gap-3 py-2">
            <input type="hidden" name="department[${key}]" value="false">
            <input type="checkbox" name="department[${key}]" value="true"
      ...
      case 'decimal':
        return `
          <div class="space-y-1">
            ...
            <input type="number" name="department[${key}]" step="${type === 'decimal' ? '0.01' : '1'}" placeholder="${label}"
              class="${baseClass}">
          </div>
        `
      case 'datetime':
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <input type="datetime-local" name="department[${key}]"
              class="${baseClass}">
          </div>
        `
      case 'text':
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <textarea name="department[${key}]" rows="3" placeholder="${label}"
      ...
      default:
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <input type="text" name="department[${key}]" placeholder="${label}"
              class="${baseClass}">
          </div>
        `
    }
  ...
  contentHTML() {
    ...
    return `
      <div class="p-4 overflow-y-auto">
        <div class="">
          ${form({
            action: Helpers.create_company_departments_path(currentCompany().id),
            method: "POST",
