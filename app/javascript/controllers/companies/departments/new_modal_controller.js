...
export default class Companies_Departments_NewModalController extends Controller {
  connect() {
  ...
  async handleSubmit(event) {
    ...
    try {
      const response = await fetchJson(Helpers.create_company_departments_path(currentCompany().id), {
        method: "POST",
      ...
      toast({
        type: "error",
        message: error.errors || "Failed to create department"
      })
    }
  }
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            ...
            <input type="text" name="department[name]" required placeholder="e.g. Sales Department"
          ...
          <div class="col-span-2 space-y-1">
            ...
            <input type="email" name="department[email]" required placeholder="e.g. sales@company.com"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500">
          </div>

          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Department Type</label>
