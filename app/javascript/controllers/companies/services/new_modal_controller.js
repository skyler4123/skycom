...
export default class Companies_Services_NewModalController extends Controller {
  connect() {
  ...
  async handleSubmit(event) {
    ...
    try {
      const response = await fetchJson(Helpers.create_company_services_path(currentCompany().id), {
        method: "POST",
      ...
      reloadThenToast({
        type: "success",
        message: `${newService.name || 'Service'} created successfully`
      })
    } catch (error) {
      toast({
        type: "error",
        message: error.errors || "Failed to create service"
      })
    }
  }
  ...
  modalHTML() {
    const typeOptions = (Enums()?.service?.business_types || []).map(t =>
      `<option value="${t.value}">${t.name.toUpperCase()}</option>`
    ).join('')

    const fields = `
      <div class="space-y-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">New Service</h2>

        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Service Name</label>
            <input type="text" name="service[name]" required placeholder="e.g. Haircut Premium"
          ...
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Service Type</label>
          ...
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Duration (minutes)</label>
            <input type="number" name="service[duration]" placeholder="e.g. 60"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500">
          </div>

          <div class="col-span-2 space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Service Code</label>
            <input type="text" name="service[code]" placeholder="e.g. SVC-001"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500">
          </div>
        </div>
