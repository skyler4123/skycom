...
export default class Companies_Brands_NewController extends Companies_LayoutController {
  /** @type {string | null} */
  categoryId = null
    ...
    switch (type) {
      case 'boolean':
        return `
          <div class="flex items-center gap-3 py-2">
            <input type="hidden" name="brand[${key}]" value="false">
            <input type="checkbox" name="brand[${key}]" value="true"
      ...
      case 'decimal':
        return `
          <div class="space-y-1">
            ...
            <input type="number" name="brand[${key}]" step="${type === 'decimal' ? '0.01' : '1'}" placeholder="${label}"
              class="${baseClass}">
          </div>
        `
      case 'datetime':
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <input type="datetime-local" name="brand[${key}]"
              class="${baseClass}">
          </div>
        `
      case 'text':
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <textarea name="brand[${key}]" rows="3" placeholder="${label}"
      ...
      default:
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <input type="text" name="brand[${key}]" placeholder="${label}"
              class="${baseClass}">
          </div>
        `
    }
  ...
  contentHTML() {
    ...
    const fields = `
      <div class="space-y-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">${translate("New Brand")}</h2>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${translate("Brand Name")}</label>
            <input type="text" name="brand[name]" required placeholder="${translate("e.g. Apple")}"
    ...
    return `
      <div class="p-4 overflow-y-auto">
        <div class="">
          ${form({
            action: Helpers.create_company_brands_path(currentCompany().id),
            method: "POST",
