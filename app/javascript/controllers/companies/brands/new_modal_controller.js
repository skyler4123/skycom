...
export default class Companies_Brands_NewModalController extends Controller {
  connect() {
  ...
  async handleSubmit(event) {
    ...
    try {
      const response = await fetchJson(Helpers.create_company_brands_path(currentCompany().id), {
        method: "POST",
      ...
      reloadThenToast({
        type: "success",
        message: `${newBrand.name || 'Brand'} created successfully`
      })
    } catch (error) {
      toast({
        type: "error",
        message: error.errors || "Failed to create brand"
      })
    }
  }
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase">Brand Name</label>
            <input type="text" name="brand[name]" required placeholder="e.g. Luxury Brand"
          ...
          <div class="space-y-1">
            ...
            <select name="brand[business_type]" required
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
