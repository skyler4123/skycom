...
export default class Companies_Brands_ShowModalController extends Controller {
  brand = null

  connect() {
    this.brand = /** @type {any} */ (window.currentBrand)
      ...
      <div class="flex items-center justify-center">
        <div class="relative w-full max-w-[640px] overflow-hidden rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl">
          ...
          <div class="flex items-center justify-between border-b border-slate-200 dark:border-gray-800 px-6 py-4">
            <h3 class="text-xl font-bold text-slate-900 dark:text-white">Brand Details</h3>
          ...
          <div class="p-6">
            ${Helpers.form({
              action: Helpers.edit_company_brand_path(currentCompany().id, b.id),
              method: "PATCH",
              attributes: "class=\"space-y-4\"",
              html: `
                <div>
                  <label class="text-[10px] font-bold text-slate-400 uppercase">Name</label>
                  <input type="text" name="brand[name]" value="${b.name}"
                ...
                <div>
                  ...
                  <select name="brand[business_type]" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
                    ${selectOptionsHTML(businessTypeFilter, b.business_type, "Select Type")}
                  </select>
                </div>

                <div>
                  <label class="text-[10px] font-bold text-slate-400 uppercase">Category</label>
                  <select name="brand[category_id]" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
                    ${selectOptionsHTML(cloneNewKey(categoryFilter, "id", "value"), b.category_id, "No Category")}
                  </select>
                </div>
