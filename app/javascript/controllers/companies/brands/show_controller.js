...
export default class Companies_Brands_ShowController extends Companies_LayoutController {
  /** @type {Brand | null} */
  brand = null
    ...
    return `
      <div class="p-4 overflow-y-auto">
        <div class="p-6 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <a href="${Helpers.company_brands_path(companyId)}" class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 mb-6">
          ...
          <div class="flex flex-col items-center gap-4 sm:flex-row sm:items-start mb-6">
            <div class="size-24 shrink-0 overflow-hidden rounded-xl border-4 border-amber-100 dark:border-amber-900/30 bg-amber-100 dark:bg-gray-800 shadow-lg flex items-center justify-center">
              <span class="material-symbols-outlined text-4xl text-amber-600 dark:text-amber-400">diamond</span>
            </div>
            <div class="flex flex-1 flex-col text-center sm:text-left">
              ...
              <p class="font-semibold text-amber-600 dark:text-amber-400">${b.description || ''}</p>
              <div class="mt-4 flex flex-wrap justify-center gap-2 sm:justify-start">
                <span class="inline-flex items-center rounded-lg bg-amber-100 dark:bg-amber-900/40 px-3 py-1 text-xs font-bold text-amber-700 dark:text-amber-300 uppercase">${b.code || 'N/A'}</span>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-6 border-t border-slate-200 dark:border-gray-800 pt-6 sm:grid-cols-2">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${Helpers.capitalize(b.business_type?.replace('_', ' ') || 'manufacturer')}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${category?.name || b.category?.name || 'N/A'}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${b.phone_number || 'N/A'}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${b.email || 'N/A'}</p>
              </div>
            </div>
          </div>
          ...
          <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-gray-800">
            <a href="${Helpers.edit_company_brand_path(companyId, b.id)}"
