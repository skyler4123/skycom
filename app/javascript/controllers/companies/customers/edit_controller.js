...
export default class Companies_Customers_EditController extends Companies_LayoutController {
  ...
  contentHTML() {
    ...
    const fields = `
      <div class="space-y-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">Edit Customer</h2>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-1">
            ...
            <select name="customer[business_type]"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
              ${selectOptionsHTML(businessTypes.map(t => ({ name: t.name === 'small_business' ? 'Small Business' : t.name, value: t.value })), c.business_type || '')}
            </select>
          </div>
