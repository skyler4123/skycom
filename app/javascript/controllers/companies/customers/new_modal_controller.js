...
export default class Companies_Customers_NewModalController extends Controller {
  connect() {
  ...
  async handleSubmit(event) {
    ...
    try {
      const response = await fetchJson(Helpers.create_company_customers_path(currentCompany().id), {
        method: "POST",
      ...
      toast({
        type: "error",
        message: error.errors || "Failed to create customer"
      })
    }
  }
    ...
    const fields = `
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Customer Type</label>
          ...
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Customer Code</label>
