...
export default class Companies_Purchases_IndexController extends Companies_LayoutController {
  // Purchases dashboard — Jira-style requisition tickets, dynamic table.
  // Search/filter controls render from the active TableConfig via the shared helpers
  // (dynamicSearchHTML / dynamicFiltersHTML).
  // Depends on BE: Companies::PurchasesController#index (list + Meilisearch q / filters[key])
  ...
  static targets = ["purchasesList"]

  /** @type {(Purchase & { total_price: number })[]} */
  purchases = []
  ...
  async connect() {
    ...
    try {
      ...
      toast({ type: "error", message: `${ translate("Failed to load purchases") }${__errDetail ? ": " + __errDetail : ""}` })
    }
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <form method="get" action="${pathname()}" class="flex flex-col lg:flex-row items-end justify-between gap-4 mb-6 w-full">
              ...
              <a href="${Helpers.new_company_purchase_path(currentCompany().id)}"
          ...
          <div class="overflow-x-auto">
            ${table({
              ...
              renderers: {
                name: (value, record) => `
                  <div class="flex items-center gap-4">
                    <div class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center shrink-0">
                      <span class="material-symbols-outlined text-emerald-600 dark:text-emerald-400 text-[18px]">shopping_cart</span>
                    </div>
                    <a href="${Helpers.company_purchase_path(currentCompany().id, record.id)}"
                      ...
                      ${value || translate("Unnamed Purchase")}
                    </a>
                  </div>
                `,
                ...
                needed_by: (value) => value
                  ? `<span class="text-sm text-slate-600 dark:text-slate-300">${new Date(value).toLocaleString()}</span>`
              ...
              renderActions: (record) => `
                <td class="py-4 px-6 text-sm text-right whitespace-nowrap">
                  <a href="${Helpers.edit_company_purchase_path(currentCompany().id, record.id)}"
