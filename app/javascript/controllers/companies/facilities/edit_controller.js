...
export default class Companies_Facilities_EditController extends Companies_LayoutController {
  /** @type {Facility | null} */
  facility = null
  ...
  async connect() {
    ...
    try {
      ...
      poll(() => {
        if (this.hasContentTarget) {
          this.contentTarget.innerHTML = `<div class="p-8 text-center text-red-600">${translate("Failed to load facility.")}</div>`
          return true
        }
        return false
      })
  ...
  contentHTML() {
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ${this.propertyMetadata.map(field => {
            ...
            switch (field.type) {
              case 'boolean':
                inputHTML = `
                  <input type="hidden" name="facility[${field.key}]" value="false">
                  <input type="checkbox" name="facility[${field.key}]" value="true" ${value ? 'checked' : ''}
                    class="rounded border-slate-300 dark:border-slate-600 text-blue-600 cursor-pointer">`
                break
              case 'integer':
                inputHTML = `<input type="number" step="1" name="facility[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
                break
              case 'decimal':
                inputHTML = `<input type="number" step="0.01" name="facility[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
                break
              case 'datetime':
                inputHTML = `<input type="datetime-local" name="facility[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
                break
              case 'text':
                inputHTML = `<textarea name="facility[${field.key}]" rows="3" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">${value ?? ''}</textarea>`
                break
              default:
                inputHTML = `<input type="text" name="facility[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
            }
            return `
              <div>
    ...
    const fields = `
      <div class="space-y-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">${translate("Edit Facility")}</h2>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            ...
            <input type="text" name="facility[name]" value="${f.name || ''}" required
          ...
          <div class="col-span-2 space-y-1">
            ...
            <textarea name="facility[description]" rows="2"
          ...
          <div class="space-y-1">
            ...
            <select name="facility[workflow_status]"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          ...
          <div class="space-y-1">
            ...
            <input type="hidden" name="facility[category_id]" value="${f.category_id}">
          </div>

          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">${translate("Phone")}</label>
            <input type="text" name="facility[phone_number]" value="${f.phone_number || ''}"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          </div>

          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">${translate("Email")}</label>
            <input type="email" name="facility[email]" value="${f.email || ''}"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-2">
          <a href="${Helpers.company_facility_path(companyId, f.id)}"
    ...
    return `
      <div class="p-4 overflow-y-auto">
        ${form({
          action: Helpers.company_facility_path(companyId, f.id),
          method: "PATCH",
