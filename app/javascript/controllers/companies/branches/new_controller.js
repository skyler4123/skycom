...
export default class Companies_Branches_NewController extends Companies_LayoutController {
  /** @type {string | null} */
  categoryId = null
    ...
    switch (type) {
      case 'boolean':
        return `
          <div class="flex items-center gap-3 py-2">
            <input type="hidden" name="branch[${key}]" value="false">
            <input type="checkbox" name="branch[${key}]" value="true"
      ...
      case 'decimal':
        return `
          <div class="space-y-1">
            ...
            <input type="number" name="branch[${key}]" step="${type === 'decimal' ? '0.01' : '1'}" placeholder="${label}"
              class="${baseClass}">
          </div>
        `
      case 'datetime':
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <input type="datetime-local" name="branch[${key}]"
              class="${baseClass}">
          </div>
        `
      case 'text':
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <textarea name="branch[${key}]" rows="3" placeholder="${label}"
      ...
      default:
        return `
          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${label}</label>
            <input type="text" name="branch[${key}]" placeholder="${label}"
              class="${baseClass}">
          </div>
        `
    }
  ...
  contentHTML() {
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${translate("Branch Name")}</label>
            <input type="text" name="branch[name]" required placeholder="${translate("e.g. Main Street Store")}"
          ...
          <div class="space-y-1">
            ...
            <input type="text" name="branch[phone_number]" placeholder="${translate("e.g. +84 123 456 789")}"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500">
          </div>

          <div class="space-y-1">
            <label class="text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">${translate("Email")}</label>
            <input type="email" name="branch[email]" placeholder="${translate("branch@example.com")}"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500">
          </div>
    ...
    return `
      <div class="p-4 overflow-y-auto">
        <div class="">
          ${form({
            action: Helpers.create_company_branches_path(currentCompany().id),
            method: "POST",
