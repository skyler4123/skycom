...
export default class Companies_Pages_EditController extends Companies_LayoutController {
  ...
  contentHTML() {
    ...
    const targetRoles = [
      { name: "Retail Cashier", value: "retail_cashier" },
      { name: "Retail Store Manager", value: "retail_store_manager" }
    ]
    const targetResolutions = [
      { name: "Mobile Portrait", value: "mobile_portrait" },
      { name: "Tablet Landscape", value: "tablet_landscape" },
      { name: "Desktop Widescreen", value: "desktop_widescreen" }
    ]
    const businessTypes = [
    ...
    const fields = `
      <div class="space-y-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">Edit Page</h2>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">Business Type</label>
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">Code</label>
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">Target Role</label>
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">Target Resolution</label>
          ...
          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">Lifecycle</label>
            <select name="page[lifecycle_status]"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none">
              <option value="active" ${p.lifecycle_status === 'active' ? 'selected' : ''}>Active</option>
              <option value="draft" ${p.lifecycle_status === 'draft' ? 'selected' : ''}>Draft</option>
              <option value="archived" ${p.lifecycle_status === 'archived' ? 'selected' : ''}>Archived</option>
            </select>
          </div>
          ...
          <div class="col-span-2 space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">Layout Manifest (JSON)</label>
