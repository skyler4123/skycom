...
export default class Companies_Suppliers_EditController extends Companies_LayoutController {
  /** @type {Supplier | null} */
  supplier = null
  ...
  async connect() {
    ...
    try {
      const response = await fetchJson(`${Helpers.company_supplier_path(companyId, recordId)}.json`)
      this.supplier = response.supplier

      if (this.supplier?.category_id) {
        const propertyMapping = currentPropertyMappings().find(m => m.category_id === this.supplier.category_id)
      ...
      poll(() => {
        if (this.hasContentTarget) {
          this.contentTarget.innerHTML = `<div class="p-8 text-center text-red-600">${translate("Failed to load supplier.")}</div>`
          return true
        }
        return false
      })
  ...
  contentHTML() {
    ...
    if (!s) return `<div class="p-8 text-center">${translate('Supplier not found.')}</div>`

    const companyId = window.location.pathname.split("/")[2]
    // Static fallbacks per docs/FLAKY_TESTS.md §6 — dropdowns always have options
    // even when the client cache hasn't loaded its enums yet.
    const businessTypes = Enums()?.supplier?.business_types || [
      { name: "Manufacturer", value: "manufacturer" },
      { name: "Distributor", value: "distributor" },
      { name: "Wholesaler", value: "wholesaler" },
      { name: "Service provider", value: "service_provider" }
    ]
    const workflowStatuses = Enums()?.supplier?.workflow_statuses || []
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ${this.propertyMetadata.map(field => {
            ...
            switch (field.type) {
              case 'boolean':
                inputHTML = `
                  <input type="hidden" name="supplier[${field.key}]" value="false">
                  <input type="checkbox" name="supplier[${field.key}]" value="true" ${value ? 'checked' : ''}
                    class="rounded border-slate-300 dark:border-slate-600 text-blue-600 cursor-pointer">`
                break
              case 'integer':
                inputHTML = `<input type="number" step="1" name="supplier[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
                break
              case 'decimal':
                inputHTML = `<input type="number" step="0.01" name="supplier[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
                break
              case 'datetime':
                inputHTML = `<input type="datetime-local" name="supplier[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
                break
              default:
                inputHTML = `<input type="text" name="supplier[${field.key}]" value="${value ?? ''}" class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">`
            }
            return `
              <div>
    ...
    const fields = `
      <div class="space-y-6">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white">${translate("Edit Supplier")}</h2>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2 space-y-1">
            ...
            <input type="text" name="supplier[name]" value="${s.name || ''}" required
          ...
          <div class="col-span-2 space-y-1">
            ...
            <textarea name="supplier[description]" rows="2"
          ...
          <div class="space-y-1">
            ...
            <select name="supplier[business_type]"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          ...
          <div class="space-y-1">
            ...
            <select name="supplier[workflow_status]"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          ...
          <div class="space-y-1">
            ...
            <input type="hidden" name="supplier[category_id]" value="${s.category_id}">
          </div>

          <div class="space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">${translate("Phone")}</label>
            <input type="text" name="supplier[phone_number]" value="${s.phone_number || ''}"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          </div>

          <div class="col-span-2 space-y-1">
            <label class="text-xs font-medium text-slate-500 dark:text-gray-400">${translate("Email")}</label>
            <input type="email" name="supplier[email]" value="${s.email || ''}"
              class="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm">
          </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-2">
          <a href="${Helpers.company_supplier_path(companyId, s.id)}"
    ...
    return `
      <div class="p-4 overflow-y-auto">
        ${form({
          action: Helpers.company_supplier_path(companyId, s.id),
          method: "PATCH",
