...
export default class Companies_Suppliers_ShowController extends Companies_LayoutController {
  /** @type {Supplier | null} */
  supplier = null
          ...
          ${this.propertyMetadata.map(field => `
            <div class="flex items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${this.formatDisplayValue(s[field.key], field.type)}</p>
              </div>
            </div>
          `).join('')}
        </div>
    ...
    return `
      <div class="p-4 overflow-y-auto">
        <div class="p-6 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
          <a href="${Helpers.company_suppliers_path(companyId)}" class="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 mb-6">
          ...
          <div class="flex flex-col items-center gap-4 sm:flex-row sm:items-start mb-6">
            <div class="size-24 shrink-0 overflow-hidden rounded-xl border-4 border-emerald-100 dark:border-emerald-900/30 bg-emerald-100 dark:bg-gray-800 shadow-lg flex items-center justify-center">
              <span class="material-symbols-outlined text-4xl text-emerald-600 dark:text-emerald-400">local_shipping</span>
            </div>
            <div class="flex flex-1 flex-col text-center sm:text-left">
              ...
              <p class="font-semibold text-emerald-600 dark:text-emerald-400">${s.description || ''}</p>
              <div class="mt-4 flex flex-wrap justify-center gap-2 sm:justify-start">
                <span class="inline-flex items-center rounded-lg bg-emerald-100 dark:bg-emerald-900/40 px-3 py-1 text-xs font-bold text-emerald-700 dark:text-emerald-300 uppercase">${s.code || translate("N/A")}</span>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-6 border-t border-slate-200 dark:border-gray-800 pt-6 sm:grid-cols-2">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${Helpers.capitalize(s.business_type?.replace('_', ' ') || 'manufacturer')}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${s.phone_number || translate("N/A")}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm font-semibold text-slate-900 dark:text-white">${s.email || translate("N/A")}</p>
              </div>
            </div>
          </div>
          ...
          <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-gray-800">
            <a href="${Helpers.edit_company_supplier_path(companyId, s.id)}"
