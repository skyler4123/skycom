...
export default class Companies_Suppliers_IndexController extends Companies_LayoutController {
  // Suppliers dashboard — table hydrates from the index JSON of the current URL.
  // Search/filter controls render from the active TableConfig via the shared helpers
  // (dynamicSearchHTML / dynamicFiltersHTML).
  // Depends on BE: Companies::SuppliersController#index (list + Meilisearch q / filters[key])
  ...
  async connect() {
    ...
    try {
      ...
      toast({ type: "error", message: `${ translate("Failed to load suppliers") }${__errDetail ? ": " + __errDetail : ""}` })
    }
  ...
  contentHTML() {
    ...
    const fallbackColumns = [
      { key: "name", name: translate("Supplier Name") },
        ...
        <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <form method="get" action="${pathname()}" class="flex flex-col lg:flex-row items-end justify-between gap-4 w-full">
              ...
              <a href="${Helpers.new_company_supplier_path(currentCompany().id)}"
            ...
            ${table({
                ...
                name: (value, record) => `
                  <div class="flex items-center gap-4">
                    <div class="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center shrink-0">
                      <span class="material-symbols-outlined text-emerald-600 dark:text-emerald-400 text-[18px]">local_shipping</span>
                    </div>
                    <a href="${Helpers.company_supplier_path(currentCompany().id, record.id)}"
                      class="font-medium text-slate-900 dark:text-white overflow-visible whitespace-normal hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors cursor-pointer">
                      ${value || translate("Unnamed Supplier")}
                    </a>
                  </div>
                `,
              ...
              renderActions: (record) => `
                <td class="py-4 px-6 text-sm text-right whitespace-nowrap">
                  <a href="${Helpers.edit_company_supplier_path(currentCompany().id, record.id)}"
                    class="inline-flex items-center justify-center p-2 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg cursor-pointer">
