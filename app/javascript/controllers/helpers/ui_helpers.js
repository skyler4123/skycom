...
/**
 * Vanilla JS + Tailwind HTML Table Generator
 * @param {HTMLElement|string} target - The DOM element or selector to mount the table into
 * @param {Object} options - Table configuration options
 * @param {Object[]} options.data - The array of objects containing record datasets
 * @param {boolean} [options.autoColumns=false] - Generate columns dynamically from first data object keys
 * @param {Object[]} [options.columns] - Explicit column definition configurations
 * @param {string} options.columns[].title - Header display text
 * @param {string} [options.columns[].field] - Object lookup key string
 * @param {string} [options.columns[].align] - Text position: 'left', 'center', or 'right'
 * @param {Function} [options.columns[].formatter] - Custom HTML renderer callback: (cellValue, rowData) => string
 */
export const table = function(target, options = {}) {
  const mountElement = typeof target === 'string' ? document.querySelector(target) : target;
  if (!mountElement) return null;

  const data = options.data || [];
  let columns = options.columns || [];

  // Handle autoColumns calculation matching your sample syntax
  if (options.autoColumns && data.length > 0 && columns.length === 0) {
  ...
  if (data.length === 0) {
    mountElement.innerHTML = `
      <div class="py-12 text-center text-sm text-slate-500 dark:text-slate-400">No records found.</div>
    `;
    return { setData: (newData) => window.table(mountElement, { ...options, data: newData }) };
  }

  // Generate pure Tailwind + semantic HTML table tree string
  const tableHTML = `
        ...
        <thead>
          <tr class="text-sm text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
            ${columns.map(col => {
              ...
              return `<th class="py-4 px-6 font-medium whitespace-nowrap ${alignment}">${col.title}</th>`;
            }).join('')}
          </tr>
        </thead>
          ...
          ${data.map(row => `
            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              ${columns.map(col => {
                ...
                // Use a customized formatter block if declared, else render the safe field text directly
                const displayContent = col.formatter ? col.formatter(rawValue, row) : (rawValue ?? '');
                
                return `<td class="py-4 px-6 text-sm text-slate-700 dark:text-slate-300 ${alignment}">${displayContent}</td>`;
              }).join('')}
            </tr>
          `).join('')}
        </tbody>
  ...
  mountElement.innerHTML = tableHTML;

  // Return a control contract object to allow updates downstream if needed
  return {
    setData: (newData) => table(mountElement, { ...options, data: newData }),
